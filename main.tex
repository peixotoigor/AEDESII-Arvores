...
\titlespacing*{\section}{0pt}{1.3ex plus .2ex minus .2ex}{0.6ex plus .1ex}
...
\begin{table}[htbp]
\centering
\caption{Diferenças conceituais de ordenação, navegação e invariantes.}
\label{tab:conceitos}
\scriptsize
...
As estruturas foram projetadas e implementadas em C++20, encapsulando ponteiros e aplicando o princípio RAII com gerenciamento explícito de memória via destrutores recursivos.
...
\end{minipage}
\caption{Evolução topológica da Árvore Trie: compartilhamento e poda de prefixos.}
\label{fig:trie_ev}
\end{figure}
...
\end{minipage}
\caption{Rastreamento de cisão e fusão de arestas na Árvore Patricia.}
...
\end{minipage}
\caption{Rastreamento de autoajuste \textit{zig-zig} na Árvore Splay.}
\label{fig:splay_ev}
\end{figure}
...
\end{minipage}
\caption{Inserção e rebalanceamento via min-heap na Árvore Treap.}
\label{fig:treap_ev}
\end{figure}
...
\end{minipage}
\caption{Rastreamento hierárquico e particionamento espacial na KD-Tree.}
\label{fig:kdtree_ev}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Comparação assintótica de operações fundamentais e complexidade espacial.}
...
Para verificar o comportamento prático das estruturas e validar as previsões assintóticas, definiu-se um protocolo experimental automatizado em C++20. O ambiente de testes baseou-se em compilador \texttt{g++ 13.3} com flags de otimização estritas (\texttt{-O3 -march=native}) sob arquitetura x86-64. Para cada ensaio, computou-se a média aritmética truncada após 10 repetições independentes antecedidas por fase de aquecimento de cache (\textit{warm-up}).
...
Os resultados empíricos consolidados das baterias de testes em C++20 são sintetizados na Tabela~\ref{tab:benchmarks}. O comportamento visual e a progressão das métricas ao longo dos diferentes parâmetros de teste encontram-se ilustrados nas Figuras~\ref{fig:bench12} e~\ref{fig:bench34}.

\begin{table}[htbp]
\centering
\caption{Resultados experimentais consolidados nos cenários de avaliação de desempenho.}
\label{tab:benchmarks}
\scriptsize
\setlength{\tabcolsep}{2.5pt}
\begin{tabularx}{\linewidth}{>{\bfseries}p{2.3cm} l *{5}{>{\centering\arraybackslash}X}}
\toprule
Cenário / Teste & Métrica Avaliada & Trie & Patricia & Splay & Treap & AVL \\
\midrule
\multirow{2}{*}{\parbox{2.2cm}{Prefixos Comuns \\ ($n=50.000$)}} 
& Tempo Busca ($\mu$s/chave) & 0.51 & \textbf{0.49} & --- & --- & 0.18 \\
& Quantidade de Nós & 476.768 & \textbf{62.732} & --- & --- & 50.000 \\
\midrule
\multirow{3}{*}{\parbox{2.2cm}{Inserção Ordenada \\ ($n=20.000$)}} 
& Tempo Inserção (ms) & --- & --- & \textbf{0.50} & 1.12 & 1.55 \\
& Altura Final & --- & --- & 20.000$^*$ & 34 & \textbf{15} \\
& Busca 2000 Chaves (ms) & --- & --- & 0.91 & 0.31 & \textbf{0.30} \\
\midrule
\multirow{2}{*}{\parbox{2.2cm}{Acessos Zipf \\ ($n=50.000$)}} 
& Latência Zipf ($\mu$s) & --- & --- & 0.80 & 0.60 & \textbf{0.36} \\
& Latência Uniforme ($\mu$s) & --- & --- & 1.02 & 0.62 & 0.40 \\
\midrule
\multirow{2}{*}{\parbox{2.2cm}{Busca $k$-NN \\ ($n=50.000$)}} 
& Tempo 2D ($\mu$s) & \multicolumn{5}{c}{\textbf{KD-Tree: 1.87 $\mu$s} \quad vs \quad Busca Linear: 170.4 $\mu$s (Ganho 91.1$\times$, 0.04\% nós)} \\
& Tempo 15D ($\mu$s) & \multicolumn{5}{c}{KD-Tree: 1584.6 $\mu$s \quad vs \quad \textbf{Busca Linear: 396.5 $\mu$s} (Linear 4.0$\times$ mais rápida)} \\
\bottomrule
\end{tabularx}
\par\vspace{2pt}{\raggedright\tiny $^*$Na Splay sob inserção puramente sequencial, a árvore assume topologia linear instantânea, mas as operações amortizadas de busca subsequentes restabelecem o equilíbrio operacional.\par}
\end{table}
...
\begin{minipage}[t]{0.49\linewidth}
  ...
  \caption{Cenário 1: Desempenho e compressão na Trie vs Patricia.}
  \label{fig:bench12}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
  \centering
  \includegraphics[width=\linewidth]{imagens/cenario2_altura_ordenada.png}
  \caption{Cenário 2: Degeneração da BST vs estabilidade AVL/Treap.}
  \label{fig:bench34}
\end{minipage}
\end{figure}
...
\begin{minipage}[t]{0.49\linewidth}
  ...
  \caption{Cenário 3: Latência total sob carga Zipf vs Uniforme.}
  \label{fig:bench_splay}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
  \centering
  \includegraphics[width=\linewidth]{imagens/cenario4_maldicao_kdtree.png}
  \caption{Cenário 4: Maldição da dimensionalidade na KD-Tree.}
...
\begin{table}[htbp]
\centering
\caption{Diretrizes de seleção de estruturas de dados hierárquicas.}
\label{tab:decisao}
\scriptsize
...
O presente estudo permitiu modelar, implementar e confrontar empiricamente cinco estruturas de dados hierárquicas avançadas frente a árvores BST e AVL. Respondendo de forma analítica às seis questões norteadoras da especificação:

\begin{enumerate}[leftmargin=*]
  \item \textbf{Diferenças estruturais primárias:} Enquanto BST e AVL baseiam-se na comparação atômica de chaves completas, Trie e Patricia exploram a decomposição em símbolos; Splay e Treap preservam a ordenação simétrica introduzindo autoajuste amortizado e balanceamento estocástico, respectivamente; e a KD-Tree emprega cortes ortogonais ciclicamente alternados no espaço euclidiano.
  \item \textbf{Liderança de desempenho por cenário:} A Árvore Patricia demonstrou inequívoca superioridade em processamento de strings com prefixos compartilhados (tempo de busca 6.4 vezes inferior à AVL e memória 70,8\% menor que a Trie). A Árvore Splay revelou-se a mais veloz para padrões de consulta com forte localidade temporal de referência (2.1 vezes mais rápida que a AVL). A KD-Tree provou-se imbatível em consultas espaciais de vizinhança em baixa dimensão, superando a varredura linear por até 141 vezes em $\mathbb{R}^2$.
  \item \textbf{Coerência empírico-assintótica:} Os ensaios computacionais convergiram rigorosamente com as classes de complexidade descritas na literatura. Observou-se a degeneração linear $\Ogrande{n}$ da BST sob entradas ordenadas, o controle estrito de altura da AVL, a estabilidade logarítmica probabilística da Treap e o efeito da maldição da dimensionalidade na KD-Tree em dimensões elevadas ($k=15$).
  \item \textbf{Dificuldades de implementação:} As estruturas de maior complexidade de desenvolvimento foram a Árvore Patricia (devido à lógica de divisão e fusão de arestas) e a KD-Tree (em razão da alternância multidimensional e da manutenção das podas euclidianas). As árvores Splay e Treap exigiram atenção redobrada na invariância das rotações binárias.
  \item \textbf{Cenários de adequação recomendada:} A Trie e a Patricia são soberanas em autocompletar, indexação de strings e roteamento IP; a Splay é recomendada para arquiteturas de caching e buffers; a Treap é excelente para conjuntos ordenados dinâmicos com divisão e união; e a KD-Tree é a escolha mandatória para bancos de dados geográficos, visão computacional e física de jogos.
  \item \textbf{Vantagens e trade-offs fundamentais:} O balanceamento determinístico estrito (AVL) elimina surpresas em pior caso, mas encarece atualizações contínuas; o balanceamento adaptativo (Splay) maximiza o throughput médio em detrimento de latências individuais previsíveis; o balanceamento probabilístico (Treap) simplifica o código de inserção às custas de depender de geradores pseudoaleatórios; e a indexação de prefixos (Trie/Patricia) troca memória por buscas assintoticamente desacopladas de $n$.
\end{enumerate}
